...
guard = nil
started_sleeping_minute = nil
guards = Hash.new { |hash, key| hash[key] = Hash.new(0) }

input.sort.each do |entry|
  if entry.end_with?("begins shift")
    guard = entry.scan(/\d+/).last.to_i
  elsif entry.end_with?("falls asleep")
    started_sleeping_minute = entry.scan(/\d+/).last.to_i
  elsif entry.end_with?("wakes up")
    woke_up_minute = entry.scan(/\d+/).last.to_i
    (started_sleeping_minute..woke_up_minute).each do |minute|
...
# Part 1
guard_id, sleeping_minutes = guards.max_by { |_, v| v.values.sum }
puts guard_id * sleeping_minutes.key(sleeping_minutes.values.max)

# Part 2
guard_id, sleeping_minutes = guards.max_by { |_, v| v.values.max }
puts guard_id * sleeping_minutes.key(sleeping_minutes.values.max)
